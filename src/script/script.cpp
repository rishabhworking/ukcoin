...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** UKC START */
    case OP_UKC_ASSET              : return "OP_UKC_ASSET";
    /** UKC END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_UKC_ASSET) { // OP_UKC_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == UKC_R) { // Check to see if UKC starts at 27 ( this->size() < 105)
                if ((*this)[28] == UKC_V)
                    if ((*this)[29] == UKC_N)
                        index = 30;
            } else {
                if ((*this)[28] == UKC_R) // Check to see if UKC starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == UKC_V)
                        if ((*this)[30] == UKC_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == UKC_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == UKC_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == UKC_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == UKC_R) {
