...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of UKCCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("UKC", type));
        BOOST_CHECK(!IsAssetNameValid("UKC", type));
        BOOST_CHECK(!IsAssetNameValid("UKCCOIN", type));

        //- Versions of UKCCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("UKC.COIN", type));
        BOOST_CHECK(IsAssetNameValid("UKC_COIN", type));
        BOOST_CHECK(IsAssetNameValid("UKCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERUKC", type));
        BOOST_CHECK(IsAssetNameValid("UKCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEUKC", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_UKCS", type));
        BOOST_CHECK(IsAssetNameValid("SEUKCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("UKC", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
