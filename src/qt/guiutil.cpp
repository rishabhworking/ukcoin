...
#include "ukcoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ukcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new UkcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new UkcoinAddressCheckValidator(parent));
}
...
bool parseUkcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ukcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!UkcoinUnits::parse(UkcoinUnits::UKC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseUkcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert ukcoin:// to ukcoin:
    //
    //    Cannot handle this later, because ukcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ukcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "ukcoin:");
    }
    QUrl uriInstance(uri);
    return parseUkcoinURI(uriInstance, out);
}

QString formatUkcoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("ukcoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(UkcoinUnits::format(UkcoinUnits::UKC, info.amount, false, UkcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openUkcoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(UKC_CONF_FILENAME);
    ...
    /* Open ukcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ukcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ukcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Ukcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("ukcoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a ukcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ukcoin\n";
        else
            optionFile << strprintf("Name=Ukcoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the ukcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef ukcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (ukcoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, ukcoinAppUrl);

    CFRelease(ukcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ukcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, ukcoinAppUrl, nullptr, nullptr);
    }
