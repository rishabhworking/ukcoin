...
const std::string UkcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString UkcoinGUI::DEFAULT_WALLET = "~Default";

UkcoinGUI::UkcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void UkcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ukcoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to UKC addresses"));
    ...
    openAction->setStatusTip(tr("Open a ukcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Ukcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void UkcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/ukcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QString tbStyleSheet = ".QToolBar {background-color : transparent; border-color: transparent; }  "
                               ".QToolButton {background-color: transparent; margin-bottom: 35px; border-color: transparent; width: 249px; color: %1; border: none;} "
        ...
        QVBoxLayout* ukcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        ukcoinLabelLayout->addWidget(label);
        ukcoinLabelLayout->addWidget(toolbar);
        ukcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ukcoinLabelLayout->addStretch(1);
...
void UkcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool UkcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool UkcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void UkcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void UkcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void UkcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void UkcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void UkcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void UkcoinGUI::showDebugWindow()
{
...
void UkcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void UkcoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void UkcoinGUI::openClicked()
{
...
void UkcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void UkcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void UkcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void UkcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void UkcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void UkcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** UKC START */
void UkcoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void UkcoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void UkcoinGUI::gotoManageAssetsPage()
{
...
void UkcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Ukcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void UkcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void UkcoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void UkcoinGUI::updateHeadersSyncProgressLabel()
{
...
void UkcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void UkcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void UkcoinGUI::changeEvent(QEvent *e)
{
...
void UkcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void UkcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void UkcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "UKC")
        msg += tr("Amount: %1\n").arg(UkcoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(UkcoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void UkcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void UkcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool UkcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool UkcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void UkcoinGUI::setHDStatus(int hdEnabled)
{
...
void UkcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void UkcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void UkcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void UkcoinGUI::detectShutdown()
{
...
void UkcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void UkcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void UkcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(UkcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void UkcoinGUI::subscribeToCoreSignals()
{
...
void UkcoinGUI::unsubscribeFromCoreSignals()
{
...
void UkcoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<UkcoinUnits::Unit> units = UkcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const UkcoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(UkcoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (UkcoinUnits::Unit u : UkcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(UkcoinUnits::name(u)), this);
...
{
    setText(UkcoinUnits::name(newUnits));
}
...
void UkcoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=UKCBTC"));
