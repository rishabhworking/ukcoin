...
The RPM spec file provided here is for Ukcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ukcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Ukcoin git repository](https://github.com/Lucky1689/ukcoin) but are not part of
...
Currently only `Source10` is used, for the example `ukcoin.conf` file.
...
in the ukcoin git repository. At present only an SVG version of the Ukcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ukcoin
...
needed when 0.12.1 is released, a proper fix is already in the Ukcoin git
master branch.
...
idea to build Ukcoin Core against a static version of that library compiled
...
The `ukcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ukcoin-server` package. At this point, most RPM based
...
The `ukcoin-server` package should have SELinux support. How to properly do
...
Ukcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ukcoin/). He has been
packaging Ukcoin for Fedora at least since 2012.
...
4. I renamed the `ukcoin` package that contains the Qt GUI to `ukcoin-core` as
that appears to be how the general population refers to it, in contrast to
`ukcoin-xt` or `ukcoin-classic`. I wanted to make sure the general population
